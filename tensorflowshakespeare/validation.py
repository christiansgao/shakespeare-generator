...
codetext, nothing, nothing = txt.read_data_files("authors/shakespeare/*.txt", validation=True)
...
with tf.Session() as sess:
    with open('results/results_jkrowling.csv', mode='a') as csvfile:
        writer = csv.writer(csvfile, delimiter=',')
        #init = tf.global_variables_initializer()
        new_saver = tf.train.import_meta_graph('checkpoints/rnn_train_1513234692-0.meta')
        new_saver.restore(sess, 'checkpoints/rnn_train_1513234692-0')
        #sess.run(init)
        ...
        for x, y_, epoch in txt.rnn_minibatch_sequencer(codetext, BATCHSIZE, SEQLEN, nb_epochs=1000):
            ...
            vali_x, vali_y, _ = next(
                txt.rnn_minibatch_sequencer(test_set_shakespeare, bsize, VALI_SEQLEN, 1))  # all data in 1 batch
            ...
            vali_x2, vali_y2, _ = next(
                txt.rnn_minibatch_sequencer(test_set_charles, bsize, VALI_SEQLEN, 1))  # all data in 1 batch
            vali_nullstate = np.zeros([bsize, INTERNALSIZE * NLAYERS])
